package ru.purecode.play.sentry.bindings;
...
public class PlayHttpRequestInterfaceBinding implements InterfaceBinding<PlayHttpRequestInterface> {
    ...
    private String buildUrl(Request request) {
    	return String.format("%s://%s", (request.secure() ? "https" : "http"), request.host()) + request.path();
    }
	...
	private void writeCookies(JsonGenerator generator, Http.Cookies cookies) throws IOException {
		generator.writeStartObject();
	...
	private void writeData(JsonGenerator generator, Request request) throws IOException {
		Map<String, String> data = Form.form().bindFromRequest(request).data();
		writeData(generator, data);
	}
}
